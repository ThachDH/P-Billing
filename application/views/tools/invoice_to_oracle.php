	...
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			<div class="ibox-head">
				...
				<div class="col-md-2 col-sm-3 col-xs-3 px-2">
						...
						<div class="row">
							...
							<div class="col-md-2 col-sm-12 col-xs-12 ml-sm-auto pl-0">
								...
								<div class="form-group">
									<button type="button" id="transfer-both" class="btn btn-sm btn-warning btn-block ml-2" data-loading-text="<i class='la la-spinner spinner'></i>Đang chuyển" data-container="body" data-toggle="popover" data-trigger="hover" data-placement="top" data-content="<b>[Mặc định]</b><br>Hoá đơn được chọn sẽ được chuyển tới ORC<br/> Phiếu thu cũng đồng thời được chuyển đi" data-html="true">
										<i class="fa fa-send"></i>
										Hoá đơn & PT
									</button>
								</div>
								<div class="form-group">
									<button type="button" id="transfer-inv" class="btn btn-sm btn-default btn-block ml-2" data-loading-text="<i class='la la-spinner spinner'></i>Đang chuyển" data-container="body" data-toggle="popover" data-trigger="hover" data-placement="top" data-content="- Chuyển hoá đơn huỷ<br/>- Chuyển hoá đơn thu sau<br/><b>[Xử lý TH1]</b><br> - Hoá đơn không có trên ORC<br/> - phiếu thu có trên ORC" data-html="true">
										<i class="fa fa-send"></i>
										Chỉ hoá đơn
									</button>
								</div>
								<div class="form-group">
									<button type="button" id="transfer-receipt" class="btn btn-sm btn-default btn-block ml-2" data-loading-text="<i class='la la-spinner spinner'></i>Đang chuyển" data-container="body" data-toggle="popover" data-trigger="hover" data-placement="top" data-content="<b>[Xử lý TH2]</b><br> - Phiếu thu không có trên ORC<br/> - Hoá đơn có trên ORC" data-html="true">
										<i class="fa fa-send"></i>
										Chỉ phiếu thu
									</button>
								</div>
...
<script type="text/javascript">
	...
	var _columnDefs = [{
		...
		{
			name: "PostToORCStatus",
			data: "PostToORCStatus",
			title: "<div class='wrap-text' style='width:65px'>Trạng thái chuyển PT</div>",
			targets: 3,
			render: function(data, type, row, meta) {
				return (!row.DRAFT_INV_NO || row.INV_TYPE == 'CRE') ? "" : (data == '2' ?
					'<span class="badge badge-danger font-weight-400">Lỗi</span>' :
					(data == '1' ?
						'<span class="badge badge-success font-weight-400">Đã chuyển</span>' :
						'<span class="badge badge-default font-weight-400">Chưa chuyển</span>'));
			}
		},
		{
			name: "PAYMENT_STATUS",
	...
	$(document).ready(function() {
		...
		$('#transfer-both').on('click', function() {
			...
			$.confirm({
				...
				type: 'orange',
				icon: 'fa fa-warning',
				content: 'Chuyển những HOÁ ĐƠN + PHIẾU THU đã được chọn!<br/>' + $('#transfer-both').data('content'),
				buttons: {
					ok: {
						text: 'Tiếp tục',
						...
						keys: ['Enter'],
						action: function() {
							postToTransfer(checkedIndexes, $('#transfer-both'), true);
						}
					},
					cancel: {
						text: 'Hủy bỏ',
			...
			$.confirm({
				...
				buttons: {
					ok: {
						...
						keys: ['Enter'],
						action: function() {
							postToTransfer(checkedIndexes, $('#transfer-inv'), false, false);
						}
					},
					cancel: {
						text: 'Hủy bỏ',
		...
		$('#transfer-receipt').on('click', function() {
			var checkedIndexes = dtDetails.rows().eq(0).filter(function(rowIdx) {
				return dtDetails.cell(rowIdx, _columnDefs.filter(p => p.name == 'IsChecked')[0].targets).data() === 1;
			}).toArray();
			...
			var nodrafts = dtDetails.rows(checkedIndexes).data().toArray().filter(p => !p.DRAFT_INV_NO);
			if (nodrafts.length > 0) {
				toastr.warning(nodrafts.map(p => p.INV_NO).join(', ') + ' không thể chuyển phiếu thu');
				return;
			}
			...
			$.confirm({
				...
				type: 'orange',
				icon: 'fa fa-warning',
				content: 'Chuyển những PHIẾU THU đã chọn!<br/>' + $('#transfer-receipt').data('content'),
				buttons: {
					ok: {
						text: 'Tiếp tục',
						btnClass: 'btn-primary',
						keys: ['Enter'],
						action: function() {
							postToTransfer(checkedIndexes, $('#transfer-receipt'), false, true);
						}
					},
					cancel: {
						text: 'Hủy bỏ',
		...
		function postToTransfer(checkedIndexes, btn, withDraft = false, onlyDraft = false) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					checkedIndexes.map(i => {
						...
						if (!datapost.onlydraft) {
							dtDetails.cell(i, _columnDefs.filter(p => p.name == 'isPosted')[0].targets).data(!isCancel ? 1 : 6);
						}
						if (datapost.withdraft) {
							dtDetails.cell(i, _columnDefs.filter(p => p.name == 'PostToORCStatus')[0].targets).data(1);
						}
					});
					...
					if (data.results) {
						...
						// draftNo
						if (datapost.withdraft) {
							var errorIndexDraft = dtDetails.rows(checkedIndexes).eq(0).filter(function(rowIdx) {
								let colIndx = _columnDefs.filter(p => p.name == 'DRAFT_INV_NO')[0].targets;
								return data.results.filter(p => p['draftNo'] == dtDetails.cell(rowIdx, colIndx).data()).length > 0;
							}).toArray();

							if (errorIndexDraft.length > 0) {
								errorIndexDraft.map(i => {
									dtDetails.cell(i, _columnDefs.filter(p => p.name == 'PostToORCStatus')[0].targets).data(2)
								});
								dtDetails.rows(dtDetails).select();
							}
						}
