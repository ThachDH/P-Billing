						...
						<div class="my-box p-3" style="height: 100%;">
							<div class="row form-group">
								<div class="col-sm-12 input-group">
									...
									<span class="input-group-addon bg-white btn mobile-hiden text-default" style="padding: 0 .5rem" title="bỏ chọn tàu" id="remove-ship-selected">
										<i class="ti-close"></i>
									</span>
...
<script type="text/javascript">
	$(document).ready(function() {
		var _colsPayment = ["STT", "TRF_CODE", "TRF_DESC", "INV_UNIT", "JobMode", "DMETHOD_CD", "CARGO_TYPE", "ISO_SZTP", "FE", "IsLocal", "QTY", "standard_rate", "extra_rate", "UNIT_RATE", "AMOUNT", "VAT_RATE", "VAT", "TAMOUNT", "CURRENCYID", "IX_CD", "CNTR_JOB_TYPE", "VAT_CHK"],
			_colCont = ["rowguid", "STT", "BILL_CHK", "CntrNo", "OprID", "CLASS_Name", "ISO_SZTP", "Status", "CJMode_CD", "DMethod_CD", "Description", "IsLocal", "JobName", "Transit_Name"],
			tblInv = $("#tbl-inv"),
		...
		function calcTotal() {
			...
			totalAmount = parseFloat(amount + totalVAT);
				...
				success: function(data) {
					...
					if (data.results && data.results.length > 0) {
						...
						for (i = 0; i < lst.length; i++) {
							...
							rows.push([
								(stt++), lst[i].TariffCode, lst[i].TariffDescription, lst[i].Unit, lst[i].JobMode == 'GO' ? "Nâng container" : (lst[i].JobMode == 'GF' ? "Hạ container" : lst[i].JobMode), lst[i].DeliveryMethod, lst[i].Cargotype, lst[i].ISO_SZTP, lst[i].FE, lst[i].IsLocal, lst[i].Quantity, lst[i].StandardTariff, lst[i].DiscountTariff, lst[i].DiscountedTariff, lst[i].Amount, lst[i].VatRate, lst[i].VATAmount, lst[i].SubAmount, lst[i].Currency, lst[i].IX_CD, lst[i].CNTR_JOB_TYPE, lst[i].VAT_CHK
							]);
						}
					}
					if (rows.length > 0) {
					...
					tblInv.DataTable({
						...
						columnDefs: [{
							...
							{
								targets: _colsPayment.getIndexs(["standard_rate", "extra_rate", "UNIT_RATE", "AMOUNT", "VAT_RATE", "VAT", "TAMOUNT"]),
								className: "text-right",
