	...
	<div class="col-xl-12">
		<div class="ibox collapsible-box">
			...
			<div class="ibox-body pt-3 pb-3 bg-f9 border-e">
				<form id="frmdata_export" method="post" action="<?= site_url(md5('Report') . '/' . md5('export_revenue_inv')); ?>">
					<div class="row border-e bg-white pb-1">
						...
						<div class="col-xs-2 col-md-2 col-lg-2 col-xl-1 mt-3">
							<div class="form-group">
						...
						<div class="col-xs-1 col-md-1 col-lg-2 col-xl-1 mt-3">
							<div class="form-group">
						...
						<div class="col-xs-3 col-md-3 col-lg-2 col-xl-2 mt-3">
							<div class="form-group">
								<label class="mb-0">Đổi từ phiếu thu</label>
							</div>
							<div class="form-group">
								<label class="radio radio-outline-primary" style="padding-right: 20px">
									<input name="isDFT_to_INV" type="radio" value="" checked>
									<span class="input-span"></span>
									Tất cả
								</label>
								<label class="radio radio-outline-primary" style="padding-right: 20px">
									<input name="isDFT_to_INV" type="radio" value="true">
									<span class="input-span"></span>
									Có
								</label>
								<label class="radio radio-outline-primary" style="padding-right: 20px">
									<input name="isDFT_to_INV" type="radio" value="false">
									<span class="input-span"></span>
									Không
								</label>
							</div>
	...
	$(document).ready(function() {
		...
		$('#contenttable').DataTable({
			...
			footerCallback: function(row, datas, start, end, display) {
				...
				if (data.length > 0) {
					sumQty = data.map(p => p[12] || 0).reduce(function(a, b) {
						return parseFloat(a) + parseFloat(b);
					}, 0);
					sumAmt = data.map(p => p[13] || 0).reduce(function(a, b) {
						return parseFloat(a) + parseFloat(b);
					}, 0);
					sumDis = data.map(p => p[14] || 0).reduce(function(a, b) {
						return parseFloat(a) + parseFloat(b);
					}, 0);
					sumVat = data.map(p => p[16] || 0).reduce(function(a, b) {
						return parseFloat(a) + parseFloat(b);
					}, 0);
					sumTamount = data.map(p => p[17] || 0).reduce(function(a, b) {
						return parseFloat(a) + parseFloat(b);
					}, 0);

					$(api.column(12).footer()).html($.formatNumber(sumQty, {
						format: "#,###.00",
						locale: "us"
					}));
					$(api.column(13).footer()).html($.formatNumber(sumAmt, {
						format: "#,###",
						locale: "us"
					}));
					$(api.column(14).footer()).html($.formatNumber(sumDis, {
						format: "#,###",
						locale: "us"
					}));
					$(api.column(16).footer()).html($.formatNumber(sumVat, {
						format: "#,###",
						locale: "us"
					}));
					$(api.column(17).footer()).html($.formatNumber(sumTamount, {
						format: "#,###",
						locale: "us"
					}));
		...
		$('#search').on('click', function() {
			...
			var formData = {
				...
				"sys": $("#sys").val(),
				"isDFT_to_INV": $("input[name='isDFT_to_INV']:checked").val()
			};
				...
				success: function(data) {
					...
					if (data.results.length > 0) {
						...
						$.each(data.results, function(idx, r) {
							...
							if (r.isDFT_to_INV == '2') {
								r.isDFT_to_INV = '<label class="checkbox checkbox-outline-ebony">' +
									'<input type="checkbox" value="1" checked="" disabled>' +
									'<span class="input-span"></span>'; +
								'</label>';
							} else {
								r.isDFT_to_INV = '<label class="checkbox checkbox-outline-ebony">' +
									'<input type="checkbox" value="0" style="display: none;" disabled>' +
									'<span class="input-span"></span>'; +
								'</label>';
							}
